...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $db->prepare("DELETE FROM testimonials WHERE id = ?")->execute([$_GET['id']]);
...
if (isset($_GET['action']) && $_GET['action'] === 'toggle' && isset($_GET['id'])) {
    $db->prepare("UPDATE testimonials SET is_active = NOT is_active WHERE id = ?")->execute([$_GET['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $db->prepare("UPDATE testimonials SET name=?, position=?, company=?, content=?, rating=?, is_active=? WHERE id=?")
           ->execute([$name, $position, $company, $content, $rating, $is_active, $id]);
        setFlash('success', 'Testimoni berhasil diperbarui!');
    } else {
        $db->prepare("INSERT INTO testimonials (name, position, company, content, rating, is_active) VALUES (?, ?, ?, ?, ?, ?)")
           ->execute([$name, $position, $company, $content, $rating, $is_active]);
...
<body>
    <aside class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="testimonials.php" class="active"><i class="fas fa-comments"></i> Testimoni</a></li>
    ...
    <div class="main-content">
        <div class="top-bar">
            <h2><i class="fas fa-comments" style="color:var(--primary);margin-right:10px;"></i>Kelola Testimoni</h2>
            <div class="user-menu">
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $t['position']; ?><?php echo $t['company'] ? '<br><small>' . $t['company'] . '</small>' : ''; ?></td>
                                <td><?php echo truncateText($t['content'], 80); ?></td>
                                <td><span class="stars"><?php for($i=0; $i<$t['rating']; $i++): ?><i class="fas fa-star"></i><?php endfor; ?></span></td>
                                ...
                                <td style="text-align:center;">
                                    <a href="?edit=<?php echo $t['id']; ?>" class="btn-sm btn-edit"><i class="fas fa-edit"></i></a>
                                    <a href="?action=toggle&id=<?php echo $t['id']; ?>" class="btn-sm btn-toggle" title="Toggle Status"><i class="fas fa-toggle-<?php echo $t['is_active'] ? 'on' : 'off'; ?>"></i></a>
                                    <a href="?action=delete&id=<?php echo $t['id']; ?>" class="btn-sm btn-delete" onclick="return confirm('Yakin ingin menghapus testimoni ini?')" title="Hapus"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if (empty($testimonials)): ?>
                            <tr><td colspan="6" style="text-align:center;color:var(--gray);padding:40px;">Belum ada testimoni. Klik "Tambah Testimoni" untuk menambahkan.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <!-- Modal Form -->
    <div class="modal-overlay" id="testimonialModal" <?php echo $editTestimonial ? 'style="display:flex;"' : ''; ?>>
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-<?php echo $editTestimonial ? 'edit' : 'plus'; ?>" style="color:var(--primary);margin-right:10px;"></i><?php echo $editTestimonial ? 'Edit' : 'Tambah'; ?> Testimoni</h3>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" value="<?php echo $editTestimonial['name'] ?? ''; ?>" required placeholder="Nama klien">
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                        <div class="form-group">
                            <label>Posisi/Jabatan</label>
                            <input type="text" name="position" value="<?php echo $editTestimonial['position'] ?? ''; ?>" placeholder="Contoh: Pemilik Rumah">
                        </div>
                        <div class="form-group">
                            <label>Perusahaan/Instansi</label>
                            <input type="text" name="company" value="<?php echo $editTestimonial['company'] ?? ''; ?>" placeholder="Nama perusahaan (opsional)">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Isi Testimoni</label>
                        <textarea name="content" rows="4" required placeholder="Tulis testimoni klien..."><?php echo $editTestimonial['content'] ?? ''; ?></textarea>
                    </div>
                    <div class="form-group">
                        <label>Rating</label>
                        <select name="rating">
                            <option value="5" <?php echo ($editTestimonial['rating'] ?? 5) == 5 ? 'selected' : ''; ?>>5 Bintang - Sangat Puas</option>
                            <option value="4" <?php echo ($editTestimonial['rating'] ?? 5) == 4 ? 'selected' : ''; ?>>4 Bintang - Puas</option>
                            <option value="3" <?php echo ($editTestimonial['rating'] ?? 5) == 3 ? 'selected' : ''; ?>>3 Bintang - Cukup</option>
                            <option value="2" <?php echo ($editTestimonial['rating'] ?? 5) == 2 ? 'selected' : ''; ?>>2 Bintang - Kurang</option>
                            <option value="1" <?php echo ($editTestimonial['rating'] ?? 5) == 1 ? 'selected' : ''; ?>>1 Bintang - Tidak Puas</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                            <input type="checkbox" name="is_active" value="1" <?php echo (!isset($editTestimonial) || $editTestimonial['is_active']) ? 'checked' : ''; ?> style="width:auto;">
                            <span>Tampilkan di website</span>
                        </label>
                    </div>
                </div>
                <div class="modal-footer">
                    <a href="testimonials.php" class="btn-cancel">Batal</a>
    ...
    <script>
        function openModal() { document.getElementById('testimonialModal').classList.add('active'); }
        function closeModal() { 
            document.getElementById('testimonialModal').classList.remove('active'); 
            <?php if (!$editTestimonial): ?>window.location.href = 'testimonials.php';<?php endif; ?>
        }
    </script>
</body>
</html>
