...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'delete' && $id) {
        $project = $db->prepare("SELECT image, gallery FROM projects WHERE id = ?")->execute([$id]);
        $proj = $db->prepare("SELECT image, gallery FROM projects WHERE id = ?");
        $proj->execute([$id]);
        $data = $proj->fetch();
        if ($data) {
            if ($data['image'] && file_exists(UPLOAD_DIR . 'gallery/' . $data['image'])) {
                unlink(UPLOAD_DIR . 'gallery/' . $data['image']);
            }
            if ($data['gallery']) {
                $gallery = json_decode($data['gallery'], true);
                foreach ($gallery as $img) {
                    if (file_exists(UPLOAD_DIR . 'gallery/' . $img)) {
                        unlink(UPLOAD_DIR . 'gallery/' . $img);
                    }
                }
            }
        }
    ...
    if ($_GET['action'] === 'toggle' && $id) {
        $db->prepare("UPDATE projects SET is_active = NOT is_active WHERE id = ?")->execute([$id]);
        setFlash('success', 'Status proyek diperbarui!');
        redirect(ADMIN_URL . '/projects.php');
    }

    if ($_GET['action'] === 'feature' && $id) {
        $db->prepare("UPDATE projects SET is_featured = NOT is_featured WHERE id = ?")->execute([$id]);
        setFlash('success', 'Status unggulan diperbarui!');
        redirect(ADMIN_URL . '/projects.php');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        if ($image) {
            $db->prepare("UPDATE projects SET title=?, category=?, description=?, client_name=?, location=?, completion_date=?, image=?, is_featured=?, is_active=? WHERE id=?")
               ->execute([$title, $category, $description, $client_name, $location, $completion_date, $image, $is_featured, $is_active, $id]);
        } else {
            $db->prepare("UPDATE projects SET title=?, category=?, description=?, client_name=?, location=?, completion_date=?, is_featured=?, is_active=? WHERE id=?")
               ->execute([$title, $category, $description, $client_name, $location, $completion_date, $is_featured, $is_active, $id]);
        }
        setFlash('success', 'Proyek berhasil diperbarui!');
    } else {
        $db->prepare("INSERT INTO projects (title, category, description, client_name, location, completion_date, image, is_featured, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
           ->execute([$title, $category, $description, $client_name, $location, $completion_date, $image, $is_featured, $is_active]);
...
<head>
    ...
    <title>Kelola Proyek - <?php echo SITE_NAME; ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-feature { background: #e8f5e9; color: var(--success); }
        .btn-feature:hover { background: var(--success); color: white; }
        .project-thumb { width: 60px; height: 45px; object-fit: cover; border-radius: 6px; border: 2px solid #e9ecef; }
        ...
        .modal-content { background: white; border-radius: var(--radius); width: 90%; max-width: 700px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 80px rgba(0,0,0,0.3); }
...
<body>
    <aside class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="projects.php" class="active"><i class="fas fa-project-diagram"></i> Proyek</a></li>
    ...
    <div class="main-content">
        <div class="top-bar">
            <h2><i class="fas fa-project-diagram" style="color:var(--primary);margin-right:10px;"></i>Kelola Proyek</h2>
            <div class="user-menu">
                ...
                <div class="card-body" style="padding:0;">
                    <table class="data-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <img src="<?php echo $project['image'] ? '../uploads/gallery/' . $project['image'] : '../assets/images/project-default.jpg'; ?>" alt="" class="project-thumb" onerror="this.src='https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=100'">
                                </td>
                                <td>
                                    <strong><?php echo $project['title']; ?></strong><br>
                                    <small style="color:var(--gray);"><?php echo $project['client_name'] ? 'Klien: ' . $project['client_name'] : ''; ?></small>
                                </td>
                                <td><?php echo $project['category'] ?: '-'; ?></td>
                                <td><?php echo $project['location'] ?: '-'; ?></td>
                                <td>
                                    <?php if ($project['is_featured']): ?><span class="status-badge status-featured" style="margin-bottom:4px;display:block;"><i class="fas fa-star"></i> Unggulan</span><?php endif; ?>
                                    <span class="status-badge <?php echo $project['is_active'] ? 'status-active' : 'status-inactive'; ?>"><?php echo $project['is_active'] ? 'Aktif' : 'Nonaktif'; ?></span>
                                </td>
                                <td style="text-align:center;">
                                    <a href="?edit=<?php echo $project['id']; ?>" class="btn-sm btn-edit"><i class="fas fa-edit"></i></a>
                                    <a href="?action=feature&id=<?php echo $project['id']; ?>" class="btn-sm btn-feature" title="Toggle Unggulan"><i class="fas fa-star"></i></a>
                                    <a href="?action=toggle&id=<?php echo $project['id']; ?>" class="btn-sm btn-toggle" title="Toggle Status"><i class="fas fa-toggle-<?php echo $project['is_active'] ? 'on' : 'off'; ?>"></i></a>
                                    <a href="?action=delete&id=<?php echo $project['id']; ?>" class="btn-sm btn-delete" onclick="return confirm('Yakin ingin menghapus proyek ini?')" title="Hapus"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if (empty($projects)): ?>
                            <tr><td colspan="6" style="text-align:center;color:var(--gray);padding:40px;">Belum ada proyek. Klik "Tambah Proyek" untuk menambahkan.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <!-- Modal Form -->
    <div class="modal-overlay" id="projectModal" <?php echo $editProject ? 'style="display:flex;"' : ''; ?>>
        <div class="modal-content">
            <div class="modal-header">
                <h3><i class="fas fa-<?php echo $editProject ? 'edit' : 'plus'; ?>" style="color:var(--primary);margin-right:10px;"></i><?php echo $editProject ? 'Edit' : 'Tambah'; ?> Proyek</h3>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" value="<?php echo $editProject['title'] ?? ''; ?>" required placeholder="Nama proyek">
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                        <div class="form-group">
                            ...
                            <select name="category">
                                ...
                                <option value="Eksterior" <?php echo ($editProject['category'] ?? '') === 'Eksterior' ? 'selected' : ''; ?>>Eksterior</option>
                                <option value="Aluminium" <?php echo ($editProject['category'] ?? '') === 'Aluminium' ? 'selected' : ''; ?>>Aluminium</option>
                                <option value="Besi" <?php echo ($editProject['category'] ?? '') === 'Besi' ? 'selected' : ''; ?>>Besi</option>
                                <option value="Stainless" <?php echo ($editProject['category'] ?? '') === 'Stainless' ? 'selected' : ''; ?>>Stainless</option>
                                <option value="Komersial" <?php echo ($editProject['category'] ?? '') === 'Komersial' ? 'selected' : ''; ?>>Komersial</option>
                                <option value="Rumah" <?php echo ($editProject['category'] ?? '') === 'Rumah' ? 'selected' : ''; ?>>Rumah</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Tanggal Selesai</label>
                            <input type="date" name="completion_date" value="<?php echo $editProject['completion_date'] ?? ''; ?>">
                        </div>
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
                        <div class="form-group">
                            <label>Nama Klien</label>
                            <input type="text" name="client_name" value="<?php echo $editProject['client_name'] ?? ''; ?>" placeholder="Nama klien/pemilik">
                        </div>
                        <div class="form-group">
                            <label>Lokasi Proyek</label>
                            <input type="text" name="location" value="<?php echo $editProject['location'] ?? ''; ?>" placeholder="Lokasi proyek">
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Deskripsi Proyek</label>
                        <textarea name="description" rows="3" placeholder="Deskripsi detail proyek..."><?php echo $editProject['description'] ?? ''; ?></textarea>
                    </div>
                    <div class="form-group">
                        ...
                        <img src="../uploads/gallery/<?php echo $editProject['image']; ?>" alt="" class="preview-img">
                        <?php endif; ?>
                    </div>
                    <div style="display:flex;gap:20px;">
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                            <input type="checkbox" name="is_featured" value="1" <?php echo ($editProject['is_featured'] ?? 0) ? 'checked' : ''; ?> style="width:auto;">
                            <span>Tampilkan sebagai proyek unggulan</span>
                        </label>
                        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                            <input type="checkbox" name="is_active" value="1" <?php echo (!isset($editProject) || $editProject['is_active']) ? 'checked' : ''; ?> style="width:auto;">
                            <span>Aktifkan proyek ini</span>
                        </label>
                    </div>
                </div>
                <div class="modal-footer">
                    <a href="projects.php" class="btn-cancel">Batal</a>
    ...
    <script>
        ...
        function closeModal() { 
            ...
            <?php if (!$editProject): ?>window.location.href = 'projects.php';<?php endif; ?>
        }
    </script>
</body>
</html>
